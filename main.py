...
TOKEN = "MTU0MzM0ODQ5Mzk0OTg3MDEyMA.GNOcLG.apJj7CZG6JOTshzRhWZTa1ezqeSZYHUs-TeG-4"
...
async def on_member_join(member):
    ...
    if recent_joins > settings["max_joins"]:
        try:
            ...
            await log_to_channel("Anti-raid", f"{member.mention} ({member.id}) banni pour multi-comptes")
        except:
            pass
        return
    ...
    if message.author.id in captcha_cache:
        ...
        if message.content.isdigit() and int(message.content) == data["result"]:
            ...
            if message.author.id not in verified:
                ...
                await message.channel.send(f"{message.author.mention} Verifie avec succes !")
...
async def verify(interaction: discord.Interaction):
    if interaction.user.id in captcha_cache:
        await interaction.response.send_message("Tu as deja une verification en cours.", ephemeral=True)
        return
    await send_captcha(interaction.user)
    await interaction.response.send_message("Verification envoyee en MP.", ephemeral=True)

@bot.tree.command(name="ban", description="[ADMIN] Bannir un membre")
...
async def ban_cmd(interaction: discord.Interaction, membre: discord.Member, raison: str = "Non specifie"):
    ...
    await log_to_channel("Ban", f"{membre.mention} ({membre.id}) banni par {interaction.user.mention}\nRaison: {raison}")

@bot.tree.command(name="kick", description="[ADMIN] Expulser un membre")
@app_commands.describe(membre="L'utilisateur", raison="Raison")
async def kick_cmd(interaction: discord.Interaction, membre: discord.Member, raison: str = "Non specifie"):
    ...
    await log_to_channel("Kick", f"{membre.mention} ({membre.id}) kick par {interaction.user.mention}\nRaison: {raison}")
...
async def clear_warnings_cmd(interaction: discord.Interaction, membre: discord.Member):
    ...
    if str(membre.id) in warnings:
        ...
        await log_to_channel("Clear Warnings", f"{membre.mention} ({membre.id}) reinitialise par {interaction.user.mention}")
    else:
        await interaction.response.send_message(f"{membre.mention} n'a aucun avertissement.", ephemeral=True)
...
async def settings_cmd(interaction: discord.Interaction, auto_verify: str = None, anti_raid: str = None, max_joins: int = None, time_window: int = None, captcha: str = None):
    ...
    await interaction.response.send_message(embed=embed, ephemeral=True)
    await log_to_channel("Settings", f"Configuration modifiee par {interaction.user.mention}")

@bot.tree.command(name="stats", description="[ADMIN] Statistiques du serveur")
...
@bot.tree.command(name="unban", description="[ADMIN] Debannir un utilisateur")
...
async def unban_cmd(interaction: discord.Interaction, user_id: str):
    ...
    try:
        ...
        await log_to_channel("Unban", f"{user.mention} ({user_id}) debanni par {interaction.user.mention}")
    except Exception as e:
        await interaction.response.send_message(f"Erreur: {e}", ephemeral=True)

@bot.tree.command(name="clear", description="[ADMIN] Supprimer des messages")
@app_commands.describe(amount="Nombre de messages (1-100)")
...
@bot.tree.command(name="info", description="[ADMIN] Informations sur un membre")
@app_commands.describe(membre="L'utilisateur")
async def info_cmd(interaction: discord.Interaction, membre: discord.Member):
    ...
    embed.add_field(name="Arrive sur le serveur", value=membre.joined_at.strftime("%d/%m/%Y %H:%M") if membre.joined_at else "Inconnu", inline=True)
...
@bot.tree.command(name="massban", description="[ADMIN] Bannir plusieurs utilisateurs")
...
@bot.tree.command(name="lockdown", description="[ADMIN] Verrouiller/Deverrouiller un salon")
@app_commands.describe(channel="Le salon", lock="True pour verrouiller, False pour deverrouiller")
